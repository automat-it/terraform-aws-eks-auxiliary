...
default_nodepool_yaml = !var.services.karpenter.enabled ? "" : <<-YAML
  ...
  %{if length(var.services.karpenter.default_nodepool_instance_cpu_manufacturer) > 0}
            - key: "karpenter.k8s.aws/instance-cpu-manufacturer"
              operator: In
              values: ${jsonencode(var.services.karpenter.default_nodepool_instance_cpu_manufacturer)}
  %{endif}
            - key: "karpenter.k8s.aws/instance-hypervisor"
              operator: In
              values: ["nitro"]
            - key: "karpenter.k8s.aws/instance-generation"
  %{if length(var.services.karpenter.default_nodepool_instance_generation) > 0}
              operator: In
              values: ${jsonencode(var.services.karpenter.default_nodepool_instance_generation)}
  %{else}
              operator: Gt
              values: ["2"]
  %{endif}
      ...
      disruption:
        consolidationPolicy: ${coalesce(var.services.karpenter.consolidation_policy, "WhenEmptyOrUnderutilized")}
