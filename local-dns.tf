...
locals {
  node_local_dns_helm_values = var.services.local-dns.enabled ? (<<-EOF
    serviceAccount:
      ...
      annotations:
        %{~if coalesce(var.services.local-dns.irsa_role_arn, try(module.node-local-dns[0].irsa_role_arn, "no_annotation")) != "no_annotation"~}
        eks.amazonaws.com/role-arn: ${coalesce(var.services.local-dns.irsa_role_arn, module.node-local-dns[0].irsa_role_arn)}
        %{~endif~}
    image:
...
module "node-local-dns" {
  ...
  create_irsa_role     = var.services.local-dns.irsa_role_arn == null ? 1 : 0
  irsa_iam_role_name   = coalesce(var.services.local-dns.irsa_iam_role_name, "${var.cluster_name}-local-dns-iam-role")
  irsa_policy_json     = var.services.local-dns.irsa_iam_policy_json
